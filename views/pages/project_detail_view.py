...
class ProjectDetailView(QWidget):
    ...
    sync_episode_details_requested = Signal(int)
    ...
    def _build_episodes_panel(self) -> None:
        ...
        self.sync_episode_details_button = QPushButton("Sync Episode Details from TMDB")
        self.sync_episode_details_button.setObjectName("secondaryButton")
        self.sync_episode_details_button.setToolTip(
            "Fetches real episode titles, air dates, runtimes, and synopses from "
            "TMDB. Requires this show to already be linked to a TMDB entry (see "
            "\"Find on TMDB\" above if it isn't)."
        )
        self.sync_episode_details_button.clicked.connect(
            lambda: self.sync_episode_details_requested.emit(self._project_id) if self._project_id else None
        )
        heading_row.addWidget(self.sync_episode_details_button)
    ...
    @staticmethod
    def _episode_tooltip_text(episode) -> str:
        lines = [episode.display_title]
        if episode.air_date is not None:
            lines.append(_format_date(episode.air_date))
        if episode.runtime_minutes is not None:
            lines.append(f"{episode.runtime_minutes} min")
        if episode.summary:
            lines.append("")
    ...
    def set_episodes(self, episodes) -> None:
        ...
        for season_number in sorted(seasons):
            ...
            for i, episode in enumerate(season_episodes):
                ...
                checkbox.setToolTip(self._episode_tooltip_text(episode))
