...
class TMDBClient:
    ...
    def get_tv_season_details(self, tv_id: int, season_number: int) -> dict[str, Any]:
        """One season's real episode list -- each entry includes
        ``name``, ``episode_number``, ``air_date``, ``runtime``, and
        ``overview`` (TMDB's own episode synopsis). Used by
        services.episode_service.sync_episodes_from_tmdb() to populate
        real per-episode data instead of the generic "Episode N"
        placeholders ensure_episodes_exist() generates when no TMDB
        sync is available for a show."""
